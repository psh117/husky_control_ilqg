...
protected:
  ...
  typedef Matrix<data_type, -1, -1> Matrixxx;
...
private:
  ...
  // % MPC %
  VectorX xt_;        ///< MPC current state at play_tick (n_)  --- %MPC%
  size_t play_tick_;  ///< MPC play tick
};
                                                 ...
                                                 int &diverge, VectorX& Vx,MatrixXX& Vxx, Vector2d& dV)
{
  ...
  for (long i = N_ -2; i>=0; i--)
  {
    ...
    if (use_u_limit_)
    {
      ...
      int QP_res;
      BoxQP<data_type, u_dim>::boxQP(QuuF, Qu, lower, upper,
